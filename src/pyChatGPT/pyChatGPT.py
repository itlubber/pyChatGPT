...
class ChatGPT:
    ...
    def __click_try_again(self) -> WebElement:
        ...
        # Try again wait for the response to be ready
        self.__verbose_print('[try_again] Waiting for completion')
        WebDriverWait(self.driver, 90).until_not(
            EC.presence_of_element_located((By.CLASS_NAME, 'result-streaming'))
        )

        # Try again check if the response is an error
        if 'text-red' in response.get_attribute('class'):
            self.__verbose_print('[try_again] Try agine response is an error')
            raise ValueError(response.text)
        
        self.__verbose_print('[try_again] Try agine response not is an error')
    ...
    def send_message(self, message: str) -> dict:
        ...
        if 'text-red' in response.get_attribute('class'):
            ...
            response = self.__click_try_again()
